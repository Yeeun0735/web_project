...
////////////////////////////////////////////////////
  passport.serializeUser((guid, done) => {
    done(null, guid.id);
  });

  passport.deserializeUser((id, done) =>  {
    Guid.findById(id, done);
  });

  passport.use('guid-signin', new LocalStrategy({
    guidnameField : 'email',
    guidpasswordField : 'password',
    passReqToCallback : true
  }, async (req, email, password, done) => {
    try {
      const guid = await Guid.findOne({email: email});
      if (guid && await guid.validatePassword(password)) {
        return done(null, guid, req.flash('success', 'Welcome!'));
      }
      return done(null, false, req.flash('danger', 'Invalid email or password'));
    } catch(err) {
      done(err);
